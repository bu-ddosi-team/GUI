...
namespace SeniorDesign_1 {
	...
	public:
		...
		Warning_DateandPatientName^ ConnectError; //Error window for being unabe to connect to the server
		ManageProfiles^ MPwin; //Window for managing profiles --> are we usig this? ********
	...
	public: 
		int numElements; //Keeps track of the number of profile names
		Form1(void)
		{
			InitializeComponent();
	        ...
	        PrevDate = "";
			fileInc = 0;
			//While loop to look through the XML file that holds the profile names and populate the drop down menu
...
//CreateGraph() plots the data that is being collected and stores it in a .csv file
private: void CreateGraph( ZedGraphControl ^zgc, int recvBytes ){
	   ...
	   double tickStart = Environment::TickCount;
			 ...
			 fileInc++;
			 string fileIncStr;          // string which will contain the result
			 ostringstream convert;   // stream used for the conversion
			 convert << fileInc;      // insert the textual representation of 'Number' in the characters in the stream
			 fileIncStr = convert.str(); // set 'Result' to the contents of the stream
...
private: System::Void button1_Click(System::Object^  sender, System::EventArgs^  e) {
			...
			// string SamPS(samplesPerStep);
			 ...
			 //Which laser diode pattern did the user select?
			 if (SimFlag == 1)
			 {
				 string SD1 = "wi";
...
private: System::Void button6_Click_1(System::Object^  sender, System::EventArgs^  e) {
			 ...
			 //If they did not, generate error window, if they did save those strings
